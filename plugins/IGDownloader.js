...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'insta') { await message.sendMessage(data, MessageType.image, { caption: Config.AFN }) }
    else if (type === 'insta') { await message.sendMessage(data, MessageType.video, { caption: Config.AFN }) }
});
//const axios = require('axios')
